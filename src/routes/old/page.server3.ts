...
function tokenizeChinesePinyin(result) {
	// Get the Pinyin transcription
	const pinyinResult = result.transcriptions.find((t) => t.script === 'Latn');
	if (!pinyinResult) {
		throw new Error('Pinyin transcription not found');
	}

	// Decode HTML entities and get the Pinyin strings
	const pinyinAccented = he.decode(pinyinResult.html);
	const pinyinNumerals = pinyinResult.text;

	// Function to split Pinyin while keeping punctuation
	const splitPinyin = (text) => {
		return text.split(/\s+|(?<=[^\w\s])|(?=[^\w\s])/).filter((token) => token.trim());
	};

	// Tokenize Pinyin (with accents) and Pinyin (with numerals)
	const pinyinAccentedTokens = splitPinyin(pinyinAccented);
	const pinyinNumeralTokens = splitPinyin(pinyinNumerals);

	// Get the Chinese sentence
	const chineseSentence = result.text;

	// Tokenize the Chinese sentence
	const chineseTokens = [];
	let chineseIndex = 0;

	pinyinNumeralTokens.forEach((token, index) => {
		if (/^[^\w\s]+$/.test(token)) {
			// If token is punctuation, add it as a separate token
			chineseTokens.push(chineseSentence[chineseIndex]);
			chineseIndex++;
		} else {
			// Count syllables (number of digits in numeral Pinyin)
			const syllableCount = (token.match(/\d/g) || []).length || 1;
			chineseTokens.push(chineseSentence.slice(chineseIndex, chineseIndex + syllableCount));
			chineseIndex += syllableCount;
		}
	});

	// Combine all information
	const tokenizedResult = chineseTokens.map((chars, index) => ({
		chars,
		pinyin: /^[^\w\s]+$/.test(pinyinAccentedTokens[index]) ? '' : pinyinAccentedTokens[index],
		pinyinNumeral: pinyinNumeralTokens[index]
	}));
...
export const load = async ({ fetch }) => {
	...
	const tokenized = tokenizeChinesePinyin(result);
